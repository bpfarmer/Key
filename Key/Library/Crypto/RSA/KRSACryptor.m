                ...
                error:(KError *)error
{
    if (!plainText)
    {
        [error addErrorWithType:KCryptErrorEncrypt
                     errorClass:[KCryptError class]];
    ...
    if ([KError error:error
      containsErrorType:KCryptErrorRSACopyKey
             errorClass:[KCryptError class]])
    {
        return nil;
    }
    ...
    if (cipherBufferSize < sizeof(nonce))
    {
        ...
        [error addErrorWithType:KCryptErrorRSATextLength
                     errorClass:[KCryptError class]];
                ...
                error:(KError *)error
{
    if (!cipherText)
    {
        [error addErrorWithType:KCryptErrorDecrypt
                     errorClass:[KCryptError class]];
                                                  ...
                                                  error:(KError *)error
{
    ...
    if (err != noErr)
    {
        [error addErrorWithType:KCryptErrorRSAGenerateKey
                     errorClass:[KCryptError class]];
    ...
    KRSACryptorKeyPair *result = [[KRSACryptorKeyPair alloc] initWithPublicKey:[self X509FormattedPublicKey:publicKeyIdentifier
                                                                                                    error:error]
                ...
                error:(KError *)error
{
    ...
    if (!strippedKey)
    {
        [error addErrorWithType:KCryptErrorRSAKeyFormat
                     errorClass:[KCryptError class]];
    ...
    {
        [error addErrorWithType:KCryptErrorRSAAddKey
                     errorClass:[KCryptError class]];
               ...
               error:(KError *)error
{
    ...
    if ([KError errorContainsErrors:error])
    {
        return;
    }
                     ...
                     error:(KError *)error
{
    ...
    if (err != noErr || !key)
    {
        [error addErrorWithType:KCryptErrorRSACopyKey
                     errorClass:[KCryptError class]];
            ...
            error:(KError *)error
{
    ...
    if ((secStatus != noErr) && (secStatus != errSecDuplicateItem))
    {
        [error addErrorWithType:KCryptErrorRSARemoveKey
                     errorClass:[KCryptError class]];
    }
}
